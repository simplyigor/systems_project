...
def get_similar_artists(artist_name):
    insert_similar("similar", artist_name)
...
def handler(event, context):
    ...
    if reply == "/start":
        ...
        if len(args) < 2:
            send_message("Укажите название исполнителя и трека через дефис. Например, /similar Hurts - Wonderful Life", chat_id)
        else:
